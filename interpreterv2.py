...
class Interpreter(InterpreterBase):
  ...
  def _var(self, tokens):
    match tokens[0]:
      case InterpreterBase.INT_DEF: #declare n variables with val 0
        for var in tokens[1:]:
          self._set_value(var, Value(Type.INT, 0))
      case InterpreterBase.STRING_DEF: #declare n variables with val ""
        for var in tokens[1:]:
          self._set_value(var, Value(Type.STRING, ""))
      case InterpreterBase.BOOL_DEF: #declare n variables with val false
        for var in tokens[1:]:
          self._set_value(var, Value(Type.BOOL, False))
      case _:
        super().error(ErrorType.TYPE_ERROR,"Invalid assignment type for variable")
  ...
  def _assign(self, tokens):
   ...
   value = self._get_value(tokens[0]) #check for NAME_ERROR when assigning variables (need var definition)
   vname = tokens[0]
   value_type = self._eval_expression(tokens[1:])
   if(value.type() != value_type.type()):
     super().error(ErrorType.TYPE_ERROR, "Invalid assignment type")
